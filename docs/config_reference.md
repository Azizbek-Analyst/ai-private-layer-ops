...
|-----|-------------|---------|
| `detector_type` | `regex`, `local`, `ner`, `presidio`, `spacy`, `spacy_trf`, `flair`, `transformers`, `scrubadub_spacy` | `regex` |
| `local_model_name` | Model name under `detectors/models/` (when `detector_type=local`); CLI: `-d --local-model NAME` | — |
| `labels` | List of labels (ner / local) | built-in list |
...
| `gliner_model` | NER model when `detector_type=ner`: Hugging Face ID or path to local directory | (default from library) |
